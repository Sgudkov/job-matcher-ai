...
import {getSearchVacancy} from '@/src/lib/api';
import {createSearch, FoundVacancy} from '@/src/types/types';
                      ...
                      <span className="text-base text-gray-700 font-medium flex items-center gap-1">
                        {type}
                          <button
                            ...
                            <div
                                className="flex justify-between items-center cursor-pointer py-2.5 border-b border-gray-300 mb-2 select-none text-[1.05rem] font-medium text-gray-700 hover:bg-gray-50 hover:rounded-lg transition-colors"
                                onClick={() => setIsSalaryExpanded((v) => !v)}>
                      ...
                      <span className="text-base text-gray-700 font-medium flex items-center gap-1">
                        {type}
                          <button
                            ...
                            <div
                                className="flex justify-between items-center cursor-pointer py-2.5 border-b border-gray-300 mb-2 select-none text-[1.05rem] font-medium text-gray-700 hover:bg-gray-50 hover:rounded-lg transition-colors"
                                onClick={() => setIsExperienceExpanded((v) => !v)}>
                            ...
                            {currentVacancies.length > 0 ? (
                                currentVacancies.map((vacancy, idx) => (
                                    <div key={vacancy.vacancy_id}
                                         ...
                                         onClick={() => handleVacancyClick(vacancy.vacancy_id.toString())}>
                                        <div className="flex justify-between items-start w-full">
                                            <div
                                                className="text-[1.3rem] font-bold m-0 mb-1.5 text-gray-800 text-left flex-1">{vacancy.title}</div>
                                            {vacancy.score > 0 && (
                                                <div
                                                    className="text-sm font-semibold text-blue-600 bg-blue-50 rounded px-3 py-1">Score: {(vacancy.score * 100).toFixed(0)}%</div>
                                            )}
                                        </div>
